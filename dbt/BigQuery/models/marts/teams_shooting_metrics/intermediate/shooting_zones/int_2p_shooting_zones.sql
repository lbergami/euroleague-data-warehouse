...
two_points_shots_zones as (
    ...
    select
        ...
        /* Shot outside the paint */
            ...
            when
                (coord_x >= -2.45 and coord_x <= 2.45) and coord_y >= 5.8
                then 'central_2p'
            when coord_x > 2.45 and coord_y > 2 then 'r_wing_2p'
            when coord_x > 2.45 and coord_y <= 2 then 'r_baseline_2p'
            /* Shot inside the paint */
            when
                (coord_x >= -2.45 and coord_x < 0)
                and (coord_y >= 2 and coord_y < 5.8)
                then 'l_high_paint_2p'
            when
                (coord_x >= 0 and coord_x <= 2.45)
                and (coord_y >= 2 and coord_y < 5.8)
                then 'r_high_paint_2p'
            when
                (coord_x >= -2.45 and coord_x < 0)
                and (coord_y >= 0 and coord_y < 2)
                then 'l_low_paint_2p'
            when
                (coord_x >= 0 and coord_x <= 2.45)
                and (coord_y >= 0 and coord_y < 2)
...
two_points_shots_zones_agg as (
    ...
    select
        ...
        case
            when
                shot_zone in ('l_baseline_2p', 'r_baseline_2p')
                then 'baseline_2p'
            when shot_zone in ('l_wing_2p', 'r_wing_2p') then 'wing_2p'
            when shot_zone = 'central_2p' then 'central_2p'
            when
                shot_zone in ('l_high_paint_2p', 'r_high_paint_2p')
                then 'high_paint_2p'
            when
                shot_zone in ('l_low_paint_2p', 'r_low_paint_2p')
                then 'low_paint_2p'
...
left join two_points_shots_zones_agg as sza
    on
        sz.season = sza.season
        and sz.game_id = sza.game_id
        and sz.shot_id = sza.shot_id
