...
dfIds = pd.read_csv(
    "gs://project-drazen-bucket/general-bucket/dfIds_season_24.csv",
    storage_options={"token": path_to_private_key},
)

lkpRawSeason24 = pd.read_csv(
    PATH_TO_BUCKET, storage_options={"token": path_to_private_key}
)
...
# Create a numeric id
dfPlayByPlay["PLAY_BY_PLAY_ID"] = (
    dfPlayByPlay.index + 1 + dfIds["PLAY_BY_PLAY_ID"].iloc[0]
)

# Export pbp data in the google storage bucket
pbp_destination_path = PATH_TO_BUCKET
dfPlayByPlay.to_csv(
    pbp_destination_path, storage_options={"token": path_to_private_key}, index=False
)
...
dfShots.to_csv(
    shots_destination_path, storage_options={"token": path_to_private_key}, index=False
)
...
dfPbPMaxId = dfPlayByPlay[["SEASON", "GAME_ID", "PLAY_BY_PLAY_ID"]][
    dfPlayByPlay.PLAY_BY_PLAY_ID == max_pbp_id
].reset_index(drop=True)
dfBsMaxId = dfBoxScores[["SEASON", "GAME_ID", "BOX_SCORES_ID"]][
    dfBoxScores.BOX_SCORES_ID == max_bs_id
].reset_index(drop=True)
dfShotsMaxId = dfShots[["SEASON", "GAME_ID", "SHOT_ID"]][
    dfShots.SHOT_ID == max_shots_id
].reset_index(drop=True)
...
dfIds.to_csv(
    PATH_TO_BUCKET, storage_options={"token": path_to_private_key}, index=False
)
